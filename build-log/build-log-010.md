...
差は**どのシェルから起動したか**にある。本文の起動確認も、今回の再ビルド直後の確認も、どちらも Claude Code の Bash ツール経由＝VS Code 拡張ホスト由来のシェル（`ELECTRON_RUN_AS_NODE=1` と `VSCODE_PID` が見える環境）で行っていた。そちらでは起動でき、素の端末では SUID チェックで落ちる。**素の端末から起動したのは今回が初めてで、元からあった地雷を初めて踏んだ**というのが実態。

### 外れた仮説：AppArmor による名前空間の制限

Ubuntu 24.04 は `kernel.apparmor_restrict_unprivileged_userns = 1`（実測）で非特権ユーザー名前空間を制限している。ここから「素の端末では namespace サンドボックスが張れず SUID にフォールバックした」と考えたが、**これは外れた。**
...
素の端末でも非特権ユーザー名前空間は作れている（Bash ツール側のシェルでも `EXIT=0`）。両者に差が無い以上、「名前空間が作れないから SUID に落ちた」という説明は成り立たない。

確定しているのはここまで。

- `chrome-sandbox` は setuid root でなかった（本文の時点から一貫して）
- 素の端末から起動すると SUID チェックで FATAL になる
- ツール経由のシェルから起動すると起動する
- `chmod 4755` で素の端末からも起動する

**なぜ一方だけ SUID サンドボックスの経路に入ったのかは未特定。** 対処法は分かったので実害は無く、深追いはしない。必要になったら `--enable-logging=stderr --v=1` でサンドボックス選択のログを両方の環境で取って比較する。
...
- **原因の仮説は、対処法が効いたことでは裏付けられない。** `chmod 4755` で直ったからといって「AppArmor が名前空間を塞いでいた」が正しいことにはならない。実際 `unshare -U` を素の端末で1回叩いただけで仮説は崩れた。**症状が消えた時点で検証をやめると、間違った理解がそのまま記録に残る**
