...
3. **仕上げ（節29〜30）：起動をコマンド1本にまとめる。** プロセスが3つに増えたので、`start-midori.sh` を書いた
...
目と声が付いた結果、**起動するプロセスが3つになった。**

| # | プロセス | ポート | 手で起動する？ |
|---|---|---|---|
| 1 | Ollama（LLM `qwen3-vl-8k`） | 11434 | 不要（systemd で常駐） |
| 2 | Irodori-TTS-Server | 8088 | 必要 |
| 3 | Open-LLM-VTuber バックエンド | 12393 | 必要 |
| 4 | Electron（Desktop Pet Mode） | — | 必要 |

ターミナルを2つ開いて別々にコマンドを打つ状態は、#012 で立てた方針の3つ目
**「できるだけ手間なく立ち上げ、本来のプロジェクトの作業の妨げにならないこと」**
に反する。相棒を起動するのに手間がかかるなら、結局使わなくなる。

なお Electron 版は、クリーンインストールで `Open-LLM-VTuber-Web` ごと消えていた。
#010 の Linux 対応をやり直して復旧させた（`release/1.2.1/linux-unpacked/`）。

`ConversationalAI/start-midori.sh` を書いた。

```bash
~/Develop/project-physical-ai/ConversationalAI/start-midori.sh
```

Irodori-TTS → LLM の事前ロード → バックエンド → Electron を順に立ち上げ、
それぞれの起動を `curl` で待ってから次へ進む。
...
**1. すでに動いているサーバーは再利用し、終了時にも止めない。** 自分で起動した
プロセスの PID だけを覚えておき、`trap` ではそれだけを片付ける。別ターミナルで
手動起動していた TTS を、うっかり殺さないため。

**2. `--check` を用意した。** 「声が出ない」ときに、どのプロセスが落ちているかを
1発で切り分けられる。今日いちばん時間を使ったのが、この手の切り分けだった。

**3. 起動のたびに `voices/` を検査する。** リファレンス音声が無いと `voice: 'none'`
相当になり、**節23の「文の区切りごとに別人の声になる」状態に戻る。** 設定ミスが
静かに起きる箇所なので、起動時に警告を出す。

**4. LLM を事前に VRAM へ載せる。** 節16で「8.288秒のうち2.084秒がロード」と
分かっているので、最初の1回からその待ちを消す。

Electron のパスはバージョン番号を決め打ちせず `release/*/linux-unpacked/` の最新を
拾う。再ビルドで `1.2.2` になっても動く。`env -u ELECTRON_RUN_AS_NODE` が必須な
理由（#010）はスクリプト内にコメントで残した。

### 30. 終了と、GPU を空けること

終了はスクリプトを動かしているターミナルで `Ctrl+C`。`trap` が Electron・
バックエンド・Irodori-TTS をまとめて止める。
...
$ ollama ps
NAME                  ID              SIZE      PROCESSOR    CONTEXT    UNTIL
qwen3-vl-8k:latest    e7b19a442e88    6.5 GB    100% GPU     8192       Forever
```

`conf.yaml` の `ollama_llm.keep_alive: -1`（無期限）が効いていて、**アプリを終了
しても Ollama は 6.5GB の VRAM を保持し続ける。** 毎回のロードを省くための設定
なので意図どおりではあるが、**SO-101 の学習を回すときは邪魔になる。**

```bash
ollama stop qwen3-vl-8k
```

方針の3つ目が「本来のプロジェクトの作業の妨げにならないこと」である以上、
**相棒AIが GPU を握ったままなのは方針違反になりうる。** LeRobot の学習前に
`ollama ps` を見る癖をつけるか、`ollama stop` まで面倒を見る停止スクリプトを
用意するか。次回の判断。
...
- **手間は、使わなくなる理由になる。** 起動プロセスが3つに増えた時点で、スクリプト1本にまとめた。相棒AIを立ち上げるのに手間がかかるなら、結局使わない。**方針「手間なく立ち上がること」は、機能を足すたびに点検し直す必要がある**
- **設定ミスが静かに起きる箇所には、検知を仕込む。** リファレンス音声が無いと声が毎回変わるが、エラーは出ない。起動スクリプトで毎回チェックするようにした。`num_ctx` の溢れも同じ性質で、**「エラーにならない失敗」こそ自動で見張る**
